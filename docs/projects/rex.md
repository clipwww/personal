...
擔任主要開發者，負責整體**全端（Nuxt2 SSR + Express.js）**架構設計與開發
...
- 重構日誌系統（Winston → Pino），效能提升 2–3 倍
- 國際化與 SEO 優化
  - 建立多語系架構
  - 動態 Sitemap 生成與快取機制
...
> 合作店家使用的平台，提供店老闆與店員操作；
功能涵蓋交易處理、行銷工具、數據分析等。
> 專案為 Hybrid App，需處理與 Native App 的互動與溝通。
...
次要開發者 -> 主要開發者，接手後負責整體**全端（Vue2 + Express.js）**架構設計與開發
...
擔任主要開發者，負責整體**全端（Nuxt2 SSR + Koa.js）**架構設計與開發

- 串接後端 API，處理錯誤與 loading 狀態
- 建立多語系架構
- 將專案從 Nuxt1 遷移至 Nuxt2
- 串接 TapPay、Stripe，支援台灣/新加坡地區付款
- 設計與實作表單流程，加速店家導入流程，提升擴展店家數量的效率
  - 多步驟表單
  - 動態欄位顯示與驗證
...
擔任主要開發者，負責整體**全端（Vue2 + Express.js）**架構設計與開發
...
擔任主要開發者，負責整體**全端（Vue3 + Express.js）**架構設計與開發

- 串接 OPEN API，處理錯誤與 loading 狀態
- 串接 LINE LIFF SDK，實作 LINE@ 入口功能
- 建立多語系架構
...
> 前端 JavaScript SDK，提供合作廠商在自己的網站或應用程式中整合 RE‧X 的功能。
> 提供合作廠商（ex: HAPPYGO）與 LINE@ LIFF 掃碼後進行交易
...
擔任主要開發者，負責以**Vue3 + Express.js**重構
